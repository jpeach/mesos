    ...
    const hashset<ContainerID>& orphans)
{
  ...
  termination.set_reason(TaskStatus::REASON_TASK_UNKNOWN);
  termination.set_message("Orphaned container");
    ...
    const Future<ContainerLimitation>& future)
{
  ...
  if (future.isReady()) {
    ...
    if (!future->resources().empty()) {
        termination->mutable_resources()->CopyFrom(future->resources());
    }
