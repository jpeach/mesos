    ...
    prid_t projectId)
{
  ...
  for (FTSENT *node = ::fts_read(tree);
    ...
    // FTS handles crossing devices, but doesn't know anything about bind
    // mounts. Linux doesn't have a direct API for detecting whether a vnode
    // is a mount point, and we prefer to not take the performance cost of
    // looking up each directory in /proc/mounts. We take advantage of the
    // contract that the path lookup checks mount crossings before checking
    // whether the rename is valid. This means that if we attempt an invalid
    // rename operation (i.e. renaming the parent directory to its child),
    // checking for EXDEV tells us whether a mount was crossed.
    //
