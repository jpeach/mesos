    ...
    const string& rootDir)
{
  ...
  auto roots = scheduledProjects.find(projectId);

  if (roots == scheduledProjects.end()) {
    scheduledProjects.put(projectId, ProjectRoots{devname.get(), {rootDir}});
  } else {
    if (roots->second.deviceName != devname.get()) {
      return Error(strings::format(
            "Conflicting device names '%s' and '%s' for project ID %s",
            roots->second.deviceName, devname.get(), projectId).get());
    }

    roots->second.directories.insert(rootDir);
  }
