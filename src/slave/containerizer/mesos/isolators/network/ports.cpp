...
// Recover the given list of containers. Note that we don't look at the
// executor resources from the ContainerState because from the perspective
// of the container, they are incomplete. That is, the resources here are
// only the resources for the executor, not the resources for the whole
// container. At this point, we don't know whether any of the tasks in th
// container have been allocated ports, so we must not start isolating it
// until we receive the resources update.
