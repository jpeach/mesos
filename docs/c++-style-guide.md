...
The Mesos codebase follows the [Google C++ Style Guide](https://google.github.io/styleguide/cppguide.html) with some notable differences, as described below. Note that the [clang-format](clang-format.html) tool can be helpful to ensure that some of the mechanical style rules are obeyed.
